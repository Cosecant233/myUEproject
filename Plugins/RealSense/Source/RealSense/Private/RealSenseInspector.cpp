...
ARealSenseInspector::ARealSenseInspector(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
    ...
    PclMesh->SetCollisionProfileName(TEXT("OverlapAll"));
    PclMesh->Mobility = EComponentMobility::Movable;
#if ENGINE_MAJOR_VERSION >= 4 && ENGINE_MINOR_VERSION >= 20
    PclMesh->SetGenerateOverlapEvents(true);
#else
...
void ARealSenseInspector::UpdatePointCloud()
{
    ...
    FVector Position[4];
    FPointCloudVertex PointCloudVertex[4];
    ...
    {
        // PointId runs horizontally then vertically along NumPoints
        if (!PclMeshData.Contains(SectionId))//新建meshdata的section
        {
        ...
        if (!PclMesh->DoesSectionExist(SectionId))
        {
            ...
            PclMesh->CreateMeshSection(SectionId, PclMeshData[SectionId]->PclVertices, PclMeshData[SectionId]->PclIndices, true, EUpdateFrequency::Frequent);
        ...
        {
            int ColumnIndex = PointId % DepthConfig.Width;//所在行

            // skip rows depending on density跳到第step-1行
            if (PointId > 0 && ColumnIndex == 0)
            {
                PointId += DepthConfig.Width * (Step - 1);
            }

            rs2::vertex Vertex0;
            rs2::vertex Vertex1 = SrcVertices[PointId];
            rs2::vertex Vertex2;
            rs2::vertex Vertex3;

            if (PointId >=DepthConfig.Width * Step+Step)
            {
                Vertex0 = SrcVertices[PointId - DepthConfig.Width * Step];          //下方一步的点
                Vertex2 = SrcVertices[PointId - Step];                              //左方一步的点
                Vertex3 = SrcVertices[PointId - DepthConfig.Width * Step - Step];   //下左方一步的点

            }
            bool IsValid = (Vertex0.z && Vertex1.z && Vertex2.z && Vertex3.z);
            if (!IsValid)
            {
                PointId += Step;
                NumInvalid++;
            ...
            bool IsBackground = (Vertex0.z > DepthMax || Vertex1.z > DepthMax || Vertex2.z > DepthMax || Vertex3.z > DepthMax);
            float ContiguousLimit = 0.03 * PclScale;
            bool IsContiguous = (
                FMath::Abs(Vertex0.z - Vertex1.z) < ContiguousLimit &&
                FMath::Abs(Vertex1.z - Vertex2.z) < ContiguousLimit &&
                FMath::Abs(Vertex2.z - Vertex3.z) < ContiguousLimit &&
                FMath::Abs(Vertex3.z - Vertex0.z) < ContiguousLimit
                ); // Prevents stretching

            if (IsBackground || !IsContiguous)
            {
                PointId += Step;
                NumInvalid++;
                continue;
            ...
            Position[0] = FVector(Vertex0.z, Vertex0.x, -Vertex0.y) * 100.0f * PclScale; // meters to mm
            Position[1] = FVector(Vertex1.z, Vertex1.x, -Vertex1.y) * 100.0f * PclScale; // meters to mm
            Position[2] = FVector(Vertex2.z, Vertex2.x, -Vertex2.y) * 100.0f * PclScale; // meters to mm
            Position[3] = FVector(Vertex3.z, Vertex3.x, -Vertex3.y) * 100.0f * PclScale; // meters to mm

            PointCloudVertex[0].Position = FVector(Position[0].X, Position[0].Y, Position[0].Z);
            PointCloudVertex[1].Position = FVector(Position[1].X, Position[1].Y, Position[1].Z);
            PointCloudVertex[2].Position = FVector(Position[2].X, Position[2].Y, Position[2].Z);
            PointCloudVertex[3].Position = FVector(Position[3].X, Position[3].Y, Position[3].Z);

            FVector Normal = FVector::CrossProduct(
                PointCloudVertex[0].Position - PointCloudVertex[1].Position,
                PointCloudVertex[0].Position - PointCloudVertex[3].Position
            );

            PointCloudVertex[0].Normal = Normal;
            PointCloudVertex[1].Normal = Normal;
            PointCloudVertex[2].Normal = Normal;
            PointCloudVertex[3].Normal = Normal;

            PointCloudVertex[0].Tangent = FVector(0, 0, 1);
            PointCloudVertex[1].Tangent = FVector(0, 0, 1);
            PointCloudVertex[2].Tangent = FVector(0, 0, 1);
            PointCloudVertex[3].Tangent = FVector(0, 0, 1);

            const auto T = SrcTexcoords[PointId];
            PointCloudVertex[0].UV0 = FVector2D(T.u, T.v);
            PointCloudVertex[1].UV0 = FVector2D(T.u, T.v);
            PointCloudVertex[2].UV0 = FVector2D(T.u, T.v);
            PointCloudVertex[3].UV0 = FVector2D(T.u, T.v);

            DstVertices[0] = PointCloudVertex[0];
            DstVertices[1] = PointCloudVertex[1];
            DstVertices[2] = PointCloudVertex[2];
            DstVertices[3] = PointCloudVertex[3];
